...
describe('DummyService', () => {
  ...
  describe('with default options (no exceptionFactory)', () => {
    beforeEach(async () => {
      const module: TestingModule = await Test.createTestingModule({
        providers: [DummyService, { provide: DUMMY_MODULE_OPTIONS, useValue: {} }],
      }).compile();
    ...
    it('throwError() should throw HttpException by default', () => {
      expect(() => service.throwError({ code: 'TEST_ERROR', message: 'Test error message' })).toThrow(HttpException);
    });
  });
  ...
  describe('with custom exceptionFactory', () => {
      ...
      const module: TestingModule = await Test.createTestingModule({
        providers: [
          DummyService,
          {
            ...
            useValue: {
              exceptionFactory: (err: { code: string; message: string }) => new CustomError(err.code, err.message),
            },
          },
        ],
      }).compile();
    ...
    it('throwError() should use the custom exceptionFactory', () => {
      expect(() => service.throwError({ code: 'CUSTOM_ERROR', message: 'Custom error' })).toThrow(CustomError);
    });
  });
});
